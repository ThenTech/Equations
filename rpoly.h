...
// Adapted from:
//		http://www.akiti.ca/rpoly_ak1_Intro.html	(27 May 2014)
//
...
#define _USE_RPOLY							//	Check if included.
...
EQUATION newEquation(size_t degree) {
	EQUATION e = {	(double*)calloc((degree + 1), sizeof(double)),
					degree,
...
void rpoly_ak1(EQUATION *eq) {
	...
	const double RADFAC = M_PI / 180;		// Degrees-to-radians conversion factor = pi/180
	...
	if (eq->coef[eq->degree] != 0) {
		...
		j = 0;
		while (eq->coef[j] == 0) {
			eq->roots[j] = newRoot(0.0, 0.0);
			N--; j++;
		}

		NN = N + 1;

		// Make a copy of the coefficients (reverse store)
		for (i = 0; i < NN; i++)   p[i] = eq->coef[NN - i - 1];

		while (N >= 1) { // Main loop
			// Start the algorithm for one zero
			if (N <= 2) {
				// Calculate the final zero or pair of zeros
				if (N < 2)  eq->roots[eq->degree - 1] = newRoot(-(p[1] / p[0]), 0.0);
				else		Quad_ak1(p[0], p[1], p[2], &eq->roots[eq->degree - 2].real, &eq->roots[eq->degree - 2].imag, &eq->roots[eq->degree - 1].real, &eq->roots[eq->degree - 1].imag);
				break;
			}
			...
			for (jj = 1; jj <= 20; jj++) {
				...
				if (NZ != 0) {
					...
					j = eq->degree - N;
					eq->roots[j] = newRoot(szr, szi);
					NN = NN - NZ;
					N = NN - 1;
					for (i = 0; i < NN; i++)   p[i] = qp[i];
					if (NZ != 1) {
						eq->roots[j + 1] = newRoot(lzr, lzi);
					} 
					break;
				} else {  // If the iteration is unsuccessful, another quadratic is chosen after restoring K
...
*/
void printInfo_ak1(EQUATION *eq) {
	printf("%s ", "Equation  :");
	toStringEq(*eq);
...
*	Returns the degree.
*/
int makeEquation_ak1(EQUATION *p, char **in, int degree) {
	if (degree > 1) {		//	Parse from cmd
		*p = newEquation(degree);
		...
		printf("%s", "Degree? ");
		scanf("%d", &degree);
		if (degree < 1) { printf("Invalid polynomial degree entered. Program terminated. \n"); return 0; }
		*p = newEquation(degree);
		readEquation(p);
		putchar('\n');
	}
